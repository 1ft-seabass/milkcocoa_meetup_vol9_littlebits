// Milkcocoa設定
var MILKCOCOA_APP_ID = "MILKCOCOA_APP_ID";
var MILKCOCOA_DATASTORE_ID = "milkcocoa_littlebits";
// Arduinoポート設定
...
app.listen(app.get('port'), function() {
    ...
    sampleDataStore.push({content : 'Hello!!!'});
    ...
    sp.on('data', function(input) {
        ...
        try {
            ...
            if( buffer == 1 ){
                ...
                sampleDataStore.push({ name : 'LittlebitsInputAction' , port : 1 });
            } else if( buffer == 2 ){
                // 2番目のポートのlittleBits入力の情報を送る
                console.log('-> LittlebitsInputAction 2');
                sampleDataStore.push({ name : 'LittlebitsInputAction' , port : 2 });
            } else if( buffer == 3 ){
                // 3番目のポートのlittleBits入力の情報を送る
                console.log('-> LittlebitsInputAction 3');
                sampleDataStore.push({ name : 'LittlebitsInputAction' , port : 3 });
            }
        } catch(e) {
            return;
        }
